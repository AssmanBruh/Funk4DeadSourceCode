...
class PlayState extends MusicBeatState
{
	...
	public var thirdPlayer:Character = null;
	...
	override public function create()
	{
		...
		dad.brightness = -0.18;
		dad.saturation = 0.04;
		boyfriend.hue = -0.0816666666666666;
		boyfriend.brightness = -0.37;
		boyfriend.saturation = 0.03;
		...
		**/
		...
		// NEW BARS!
		playerUIBar = new F4DHealthBar(PLAYER, 3, "Xx_ValentinoPalaciosJosiMartinezGameFullPedoSacadaDeHuano_xX", boyfriend.isInfected ? INFECTED : SURVIVOR);
		playerUIBar.dahealth = this.health;
		playerUIBar.cameras = [camHUD];
		UiBarGroup.add(playerUIBar);

		var charUINAME:Array<String> = [];
		var file:String = Paths.txt('characterUIName'); //char ui names
		if (OpenFlAssets.exists(file)) {
			charUINAME = CoolUtil.coolTextFile(file);
		}
		var leName = "ass";
		for (i in 0...charUINAME.length)
		{
			var leChar:Array<String> = charUINAME[i].split(":");
			if (dad.curCharacter == leChar[1]){
				leName = leChar[2];
			}
		}
		
		opponentUIBar = new F4DHealthBar(OPPONENT, 4, leName, dad.isInfected ? INFECTED : SURVIVOR);
	...
	**/
	...
	override public function update(elapsed:Float)
	{
		...
		if (camZooming)
		{
			...
			var angled = Math.sin(0.798*camAngleElapsed)*(Conductor.stepCrochet * 0.00005);
			// angled = FlxMath.lerp(angled, 0, CoolUtil.boundTo(elapsed * 2.4 * cameraSpeed * playbackRate,0,1));
			// FlxG.camera.angle = FlxMath.lerp(FlxG.camera.angle, angled, CoolUtil.boundTo(elapsed*2.22, 0, 1));
			FlxG.camera.angle = angled;
	...
	**/
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			// case 'Add Another Character': //hello pelonchas
			case 'Add Third Player':
				// assman credits here :V
				var val1 = value1.split(", ");
				trace(val1);
				trace(val1[0]);
				trace(val1[1]);
				trace(val1[2]);
				thirdPlayer = new Character(Std.parseFloat(val1[1]), Std.parseFloat(val1[2]), val1[0]);
				thirdPlayer.characterType = value2.toLowerCase();
				trace(thirdPlayer.characterType);
				switch (value2.toLowerCase()){
					case "player":
						startCharacterPos(thirdPlayer);
						boyfriendGroup.add(thirdPlayer);
					case "enemie":
						startCharacterPos(thirdPlayer, true);
						dadGroup.add(thirdPlayer);
					case "gf":
						startCharacterPos(thirdPlayer);
						gfGroup.add(thirdPlayer);
				}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.gfNote) {
			char = gf;
		}else if (daNote.thirdPlayer){
			if (thirdPlayer != null)
			{
				if (thirdPlayer.characterType == "player")
					char = thirdPlayer;
			}
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if(note.gfNote) {
				char = gf;
			}else if (note.thirdPlayer){
				if (thirdPlayer != null){
					if (thirdPlayer.characterType == "enemie"){
						char = thirdPlayer;
					}
				}
			}
		...
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if(note.gfNote)
				{
					...
					if (thirdPlayer != null){
					if (thirdPlayer.characterType == "player"){
					thirdPlayer.playAnim(animToPlay+note.animSuffix, true);
					thirdPlayer.holdTimer = 0;
					}
					}else{
						trace("no effect");
					}
